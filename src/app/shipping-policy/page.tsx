  ...
  useEffect(() => {
    const loadSettings = async () => {
      try {
        ...
        console.error("Error loading shipping policy page settings:", err);
      }
    };
    loadSettings();
  }, []);

  const config = settings.shipping || DEFAULT_PAGE_SETTINGS.shipping;

  return (
    <Box>
      <PageBanner
        title={config.bannerTitle || "SHIPPING & DELIVERY POLICY"}
      ...
      <Container maxWidth="md" sx={{ py: 8 }}>
        <Paper sx={{ p: { xs: 3, md: 5 }, borderRadius: 3, border: "1px solid #e2e8f0" }} elevation={0}>
          <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, mb: 2 }}>
            <LocalShippingIcon sx={{ fontSize: 36, color: BRAND.navy }} />
            <Typography variant="h4" sx={{ fontWeight: 800, color: BRAND.navy }}>
              {config.bannerTitle || "Shipping & Delivery Policy"}
            </Typography>
          </Box>
          {config.lastUpdated && (
